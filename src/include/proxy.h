...
struct ncclProxyOps {
  ...
  } tmpArgsPerConn[NCCL_MAX_PROXY_CONNECTIONS];    // NCCL_MAX_PROXY_CONNECTIONS ？// int tpLocalRank 用于索引

  int tpLocalRankList[NCCL_MAX_PROXY_CONNECTIONS]; // tpRank is being used
  int tpLocalRankCount;                            // num of tpRank is being used
  uint8_t usedMap[USED_MAP_SIZE_IN_UINT8];         // if tpRank is being used
};
