...
static ncclResult_t ncclLocalOpAppend(struct ncclComm* comm, struct ncclProxyConnector* proxyConn, struct ncclProxyOp* proxyOp) {
  ...
  if (should_flush) {
    ...
    for (int i = 0; i < proxyOps->count; i++) {
      ...
      int connTpRank = conn->tpLocalRank;
      int shared = conn->shared;

      struct ncclProxyArgs** pArgs = &proxyOps->tmpArgsPerConn[connTpRank].args;
      struct ncclProxyArgs** pLast = &proxyOps->tmpArgsPerConn[connTpRank].lastArgs;

      if (!TEST_BIT(proxyOps->usedMap, connTpRank)) {
        // first time
        SET_BIT(proxyOps->usedMap, connTpRank);
        proxyOps->tpLocalRankList[proxyOps->tpLocalRankCount++] = connTpRank;
        NCCLCHECK(ncclProxyOpToArgs(op, proxyOps->tmpArgsPerConn[connTpRank].args, 0));
        *pLast = *pArgs;
      } else {
    ...
    for (int i = 0; i < proxyOps->tpLocalRankCount; i++) {
        int tpRank = proxyOps->tpLocalRankList[i];
        struct ncclProxyArgs* args = proxyOps->tmpArgsPerConn[tpRank].args;
        if (!args) continue;

        int ring_id = tpRank % N_PROXY_PROGRESS;
        batch[ring_id][batch_count[ring_id]++] = args;

        proxyOps->tmpArgsPerConn[tpRank].args = NULL;
        proxyOps->tmpArgsPerConn[tpRank].lastArgs = NULL;
    }
